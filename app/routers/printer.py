...
# --- AUTH (YETKİLENDİRME) BÖLÜMÜ ---
api_key_header = APIKeyHeader(name="X-API-Key", auto_error=True)

def verify_token(api_key: str = Security(api_key_header)):
    """Header'da gelen X-API-Key'in config'deki token ile eşleştiğini doğrular."""
    if api_key != settings.API_TOKEN:
        raise HTTPException(status_code=401, detail="Geçersiz API Token")
    return api_key

# Router'a yetkilendirmeyi bağlıyoruz. Artık bu dosyadaki hiçbir uca tokensız erişilemez.
router = APIRouter(prefix="/printer", tags=["Printer"], dependencies=[Depends(verify_token)])
_start_time = time.monotonic()
...
# --- 1. TEXT BASKI UCU ---
@router.post("/print/text", response_model=PrintResponse, status_code=202)
async def print_text(req: PrintRequest, session: Session = Depends(get_session)):
    req.content_type = "text"  # İsteğin tipini zorunlu olarak text yapıyoruz
    job = await submit_print_job(session, req)
    return PrintResponse(
        job_id=job.id,
        status=job.status,
...
# --- 2. GÖRSEL/RESİM BASKI UCU ---
@router.post("/print/image", response_model=PrintResponse, status_code=202)
async def print_image(req: PrintRequest, session: Session = Depends(get_session)):
    req.content_type = "image"  # İsteğin tipini zorunlu olarak image yapıyoruz
    job = await submit_print_job(session, req)
    return PrintResponse(
        job_id=job.id,
        status=job.status,
        message="Image job queued. Use GET /logs to track progress.",
        idempotency_key=req.idempotency_key,
    )

# --- 3. QR BASKI UCU ---
@router.post("/print/qr", response_model=PrintResponse, status_code=202)
async def print_qr(req: PrintRequest, session: Session = Depends(get_session)):
    req.content_type = "qr"
    job = await submit_print_job(session, req)
    return PrintResponse(
        job_id=job.id,
        status=job.status,
        message="QR job queued. Use GET /logs to track progress.",
        idempotency_key=req.idempotency_key,
    )
